defmodule SocialCrowdWorkWeb.AdminAuth do
  ...
  @remember_me_cookie "_social_crowd_work_web_admin_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      SocialCrowdWorkWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      SocialCrowdWorkWeb.Endpoint.broadcast(admin_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule SocialCrowdWorkWeb.PageLive do
        use SocialCrowdWorkWeb, :live_view

        on_mount {SocialCrowdWorkWeb.AdminAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SocialCrowdWorkWeb.AdminAuth, :require_authenticated}] do
