defmodule SocialCrowdWorkWeb.AdminAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SocialCrowdWorkWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    ...
    test "redirects to the admin panel when admin is already logged in", %{
      conn: conn,
      admin: admin
    } do
      conn =
  ...
  describe "logout_admin/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SocialCrowdWorkWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SocialCrowdWorkWeb.Endpoint.subscribe("admins_sessions:#{Base.url_encode64(token)}")
      end
